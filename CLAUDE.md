...
The game repository lives at `../` (parent repo root). The game's own `CLAUDE.md` is the authoritative source for game mechanics, role names, campaign structure and data. **Always cross-reference game data before writing website content.** Role names, counts and campaign structure change in the game repo first.
